...
## Reference
Chapter 10.2, Hosoe, N, Gasawa, K, and Hashimoto, H. Handbook of Computable General Equilibrium Modeling. University of Tokyo Press, Tokyo, Japan, 2004.
...
$$
Td = tau\_d \cdot (\sum_{h \in \mathcal{D}_{h}} pf_{h} \cdot FF_{h} + 0 + 0)
$$
...
$$
Sp = ssp \cdot (\sum_{h \in \mathcal{D}_{h}} pf_{h} \cdot FF_{h} + 0 + 0)
$$
...
$$
Xp_{i} = alpha_{i} \cdot (\sum_{h \in \mathcal{D}_{h}} pf_{h} \cdot FF_{h} - Sp - Td + 0 + 0) / pq_{i}
$$
...
$$
E_{i} / E0_{i} = (pWe_{i} / pWe0_{i})^{-sigma_i}
$$

`foreign_trade.eqfm[BRD]`

$$
M_{i} / M0_{i} = (pWm_{i} / pWm0_{i})^{psi_i}
$$

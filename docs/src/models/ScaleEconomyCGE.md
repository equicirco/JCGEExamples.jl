...
## Reference
Chapter 10.6, Hosoe, N, Gasawa, K, and Hashimoto, H. Handbook of Computable General Equilibrium Modeling. University of Tokyo Press, Tokyo, Japan, 2004.
...
```math
pz_{i} = ay_{i} \cdot py_{i} + \sum_{j \in \mathcal{D}_{j}} ax_{j,i} \cdot pq_{j} + FC_{i} / Z_{i}
```
...
```math
Td = tau\_d \cdot (\sum_{h \in \mathcal{D}_{h}} pf_{h} \cdot FF_{h} + 0 + \sum_{i \in \mathcal{D}_{i}} FC_{i})
```
...
```math
Sp = ssp \cdot (\sum_{h \in \mathcal{D}_{h}} pf_{h} \cdot FF_{h} + 0 + \sum_{i \in \mathcal{D}_{i}} FC_{i})
```
...
```math
Xp_{i} = alpha_{i} \cdot (\sum_{h \in \mathcal{D}_{h}} pf_{h} \cdot FF_{h} - Sp - Td + 0 + \sum_{j \in \mathcal{D}_{j}} FC_{j}) / pq_{i}
```
